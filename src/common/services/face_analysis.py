...
class FaceAnalysis:
    ...
    def detect_faces(self) -> List[Dict[str, Union[str, float]]]:
        self.__logger.log.info(
            f"Iniciada detecção de faces para imagem: {self.__image_name}"
        )
        ...
        try:
            ...
            self.__logger.log.info(
                f"Detecção de faces finalizada com sucesso.\n"
                f"Detectado: {detected_images}"
            )
            return detected_images
        except ClientError as client_error:
            message = (
                f"Erro ao chamar serviços AWS para detecção de faces: "
            ...
            raise GenericException(
                message=message, status_code=500, payload={"generic_error": message}
            )

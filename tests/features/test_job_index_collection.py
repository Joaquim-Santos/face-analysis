...
class TestJobIndexCollection:
    ...
    def test_job_finished_with_success_if_bucket_has_no_images_to_index(
        fake_get_files_names: MagicMock,
    ) -> None:
    ...
    def test_job_finished_with_success_if_has_one_retry_after_client_error(
        ...
        with patch.object(S3Service, "get_files_names") as fake_get_files_names:
            fake_get_files_names.side_effect = [["input/fake_image.png"], all_files]
    ...
    def test_job_failed_if_exceed_max_retries_due_generic_error(
        fake_sleep: MagicMock, fake_get_files_names: MagicMock
    ) -> None:
